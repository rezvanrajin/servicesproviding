...
class SupportController extends Controller
{
    ...
    public function store(Request $request)
    {
        if ($request->isMethod("post")) {
            ...
            $replayMail->user_id = Auth::guard('api')->user()->id;
            $replayMail->provider_id = 0;
...
public function destroy(Mailbox $buyermail)
{
    if (is_null($buyermail)) {
...
public function show(Mailbox $buyermail)
{

    return response()->json($buyermail);
}
